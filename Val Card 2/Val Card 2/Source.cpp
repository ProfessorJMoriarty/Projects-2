...
int main(int argc, char **argv) {
	...
	sample = al_load_sample("cat_purr.wav");
	if (sample == NULL)
	...
	al_draw_text(font,al_map_rgb(255,255,30),150,150,ALLEGRO_ALIGN_CENTER,"You're a Star");
	...
	while (1)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			bouncer_x = 150 + 50 * (2.6*cos(t) - (1.4*cos((1.6*t) / .6)));
			bouncer_y = 150 + 50 * (2.6*sin(t) - (1.4*sin((1.6*t) / .6)));
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			al_clear_to_color(al_map_rgb(150, 150,t));
			al_set_target_bitmap(al_get_backbuffer(display));
